...
export function shortenAddress(input: string | PublicKey | null | undefined, start = 4, end = 4): string {
...
export function useShortAddress(input: string | PublicKey | null | undefined, start = 4, end = 4) {
...
async function fetchSplBalanceByMint(connection: any, ownerPk: PublicKey, mintStr: string) {
  ...
  const ata = await getAssociatedTokenAddress(mint, ownerPk, false, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID)
...
export function WalletConnectButton({ className = "" }) {
    ...
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          <div
            onClick={() => navigator.clipboard.writeText(publicKeyString ? publicKeyString : "")}
            className="px-3 py-2 opacity-70 font-mono text-xs cursor-pointer hover:text-[#a9a9a9] transition-colors"
          >
            {short12}
          </div>
  ...
  return (
    ...
    >
      ...
      <span className="hidden sm:inline text-white">{connecting ? "Connecting..." : "Connect Wallet"}</span>
